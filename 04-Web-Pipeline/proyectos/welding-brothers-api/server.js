...
const BRANDS = {
  ...
  'leaderforge-digital': {
    displayName: 'LeaderForge Digital',
    telegramToken: process.env.TELEGRAM_BOT_TOKEN_LF,
    telegramChatId: process.env.TELEGRAM_CHAT_ID_LF,
    sheetTab: 'LeaderforgeDigital',
    r2Folder: 'leaderforge-digital/leads',
    phones: ['+18584658919'],
    forwardTo: process.env.LEADERFORGE_FORWARD_TO || '+19517336105',
  },
};
...
app.post('/telnyx/incoming', async (req, res) => {
  try {
    ...
    if (req.body.CallSid && !req.body.CallStatus) {
      ...
      res.set('Content-Type', 'text/xml');
      const dialTo = brand.forwardTo || to;
      return res.send(`<?xml version="1.0" encoding="UTF-8"?><Response><Dial>${dialTo}</Dial></Response>`);
    }
...
// --- Demo Page Tracking Pixel ---
// Served when a prospect opens their personalized demo page.
// Sends Telegram notification so Daniel can follow up immediately.
app.get('/api/demo-view', async (req, res) => {
  try {
    const leadId = req.query.id || 'unknown';
    const leadName = req.query.name || 'Unknown';
    const leadCity = req.query.city || 'Unknown';
    const ua = (req.get('user-agent') || '').toLowerCase();

    // Skip automated/bot traffic (Playwright, headless browsers, crawlers)
    const isBot = [
      'headless', 'playwright', 'puppeteer', 'selenium', 'chrome-lighthouse',
      'googlebot', 'bingbot', 'slurp', 'duckduckbot', 'baiduspider',
      'facebookexternalhit', 'twitterbot', 'bot/', 'crawler', 'scanner',
      'netlify', 'prerender', ' preview', 'scan', 'check',
    ].some(kw => ua.includes(kw));

    if (isBot) {
      console.log(`Demo-view BLOCKED (bot): ${decodeURIComponent(leadName)} (${decodeURIComponent(leadId)}) UA: ${ua.substring(0, 80)}`);
    ...
    const msg = [
      `👀 *Demo Page Viewed*`,
      '',
      `🏢 *Business:* ${decodeURIComponent(leadName)}`,
      `📍 *City:* ${decodeURIComponent(leadCity)}`,
      `🆔 *Lead:* ${decodeURIComponent(leadId)}`,
      `🕐 *Time:* ${getLAFormatted()}`,
    ].join('\n');

    // Send to LeaderForge Digital Telegram (Daniel's phone)
    const brand = BRANDS['leaderforge-digital'];
    const baseUrl = `https://api.telegram.org/bot${brand.telegramToken}`;
    ...
    }).catch(e => console.error('Demo-view Telegram failed:', e.message));

    console.log(`Demo viewed: ${decodeURIComponent(leadName)} (${decodeURIComponent(leadId)})`);
  } catch (err) {
    console.error('Demo-view error:', err.message);
  }
...
// --- Health Check ---
app.get('/', (req, res) => {
  res.json({ status: 'ok', service: 'Welding Brothers + Roof Kings + LeaderForge Digital API', brands: Object.keys(BRANDS) });
});
